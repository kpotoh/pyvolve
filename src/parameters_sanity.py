...
class MutSel_Sanity(ParametersSanity):
    '''
        ...
        Required checks:
            ...
            2. mutation rates
            3. [Optional] population size, N_e. *May only be provided in concert with fitness values, which will then be assumed to be unscaled fitnesses and will be re-computed to scaled fitness values as 2Ne_s*
    '''
    ...
    def _sanity_state_freqs_fitness(self):
        ...
        # Check population size Ne and re-scale fitness accordingly
        if "Ne" in self.params:
            if self.params["calc_by_freqs"] is True:
                warn("\n\nThe Ne argument can only be used if fitnesses are provided, not stationary frequencies. The provided value will be ignored.")
            else:
                try:
                    self.params["Ne"] = float(self.params["Ne"])
                except:
                    raise TypeError("\n\nThe Ne value must be a float/integer.")
                self.params["fitness"] *= (2*self.params["Ne"])
